...
// 占位卡牌（暂未实现）
const placeholderCards: Partial<Card>[] = [
  {
    id: 'card_placeholder_001',
    name: '农夫',
    keywords: ['居民', '务农']
  },
  {
    id: 'card_placeholder_002',
    name: '锻炉',
    keywords: ['建筑']
  },
  {
    id: 'card_placeholder_003',
    name: '板甲',
    keywords: ['护甲']
  },
  {
    id: 'card_placeholder_004',
    name: '森林',
    keywords: ['自然']
  }
]
...
export function initializeCardDatabase() {
  CardDatabase.clear()
  CardDatabase.registerAll(cardDefinitions)
}
...
export function createDeck(): Card[] {
  ...
  cardDefinitions.forEach((cardDef) => {
    const card = CardDatabase.get(cardDef.id)
    if (card) {
      // 每张卡只有1张
